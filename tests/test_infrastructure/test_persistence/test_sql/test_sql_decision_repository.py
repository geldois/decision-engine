...
def test_sql_decision_repository_returns_saved_decision() -> None:
    ...
    decision = DecisionEngine().decide(event=event, rules=[rule])
...
def test_sql_decision_repository_returns_decision_when_id_exists() -> None:
    ...
    assert returned_decision

    assert returned_decision == decision

    assert returned_decision.is_structurally_equal(other=decision)
...
def test_sql_decision_repository_returns_list_of_decisions() -> None:
    ...
    for d in decisions:
        if d == decision:
            assert d.is_structurally_equal(other=decision)
