...
from decision_engine.interface.http.schemas.responses.http_error_response import HTTPErrorResponse

_MAPPING: dict[ErrorCode, int] = {
    ErrorCode.CONDITION_INVALID: status.HTTP_422_UNPROCESSABLE_CONTENT,
    ErrorCode.CONDITION_MISSING_FIELDS: status.HTTP_422_UNPROCESSABLE_CONTENT,
    ErrorCode.CONDITION_OPERATOR_EMPTY: status.HTTP_422_UNPROCESSABLE_CONTENT,
    ErrorCode.CONDITION_OPERATOR_INVALID: status.HTTP_422_UNPROCESSABLE_CONTENT,
    ErrorCode.CONDITION_TYPE_INVALID: status.HTTP_422_UNPROCESSABLE_CONTENT,
    ErrorCode.CONDITION_VALUE_EMPTY: status.HTTP_422_UNPROCESSABLE_CONTENT,
    ...
    ErrorCode.EVENT_OCCURRED_AT_INVALID: status.HTTP_422_UNPROCESSABLE_CONTENT,
    ...
    ErrorCode.RULE_PRIORITY_INVALID: status.HTTP_422_UNPROCESSABLE_CONTENT,
    ErrorCode.RULE_NOT_FOUND: status.HTTP_404_NOT_FOUND,
}


def map_http_error_code(error_code: ErrorCode) -> int:
    try:
...
def map_http_exception(
    ...
    if isinstance(exception, DomainException):
        ...
        return HTTPException(
            status_code=map_http_error_code(error_code=exception.error_code),
