...
class SqlDecisionRepository(DecisionRepositoryContract):
    ...
    def convert_decision_to_decision_model(
        self, 
        decision: Decision
    ) -> DecisionModel:
        decision_model = DecisionModel(
    ...
    def get_by_id(
        ...
        if decision_model:
            return self.convert_decision_model_to_decision(decision_model = decision_model)
    ...
    def list_all(self) -> list[Decision]:
        ...
        for decision_model in decision_models:
            decision = self.convert_decision_model_to_decision(decision_model = decision_model)
