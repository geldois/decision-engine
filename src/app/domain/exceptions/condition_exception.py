...
class ConditionException(DomainException):
    ...
    @classmethod
    def condition_type_is_invalid(
        cls, details: dict[str, Any] | None = None
    ) -> ConditionException:
        return cls(
            message="Condition type is invalid",
            error_code=ErrorCode.CONDITION_TYPE_INVALID,
            details=details,
        )
    ...
    def missing_fields(
        ...
        return cls(
            message="Condition fields are missing",
            error_code=ErrorCode.CONDITION_MISSING_FIELDS,
            details=details,
        )
